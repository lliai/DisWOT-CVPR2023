...
def find_measures_arrays(
    ...
    if dataload == 'random':
        inputs, targets = get_some_data(trainloader,
                                        num_batches=num_imgs_or_batches,
                                        device=device)
    elif dataload == 'grasp':
...
def find_measures(
    ...
    if measure_names[0] in ['flops', 'params']:
        ...
        model_stats = get_model_stats(net_orig,
    ...
    for k, v in measures_arr.items():
        if k == 'jacov' or k == 'epe_nas' or k == 'nwot' or k == 'zen' or k == 'diswot':
            measure_score = v
        else:
