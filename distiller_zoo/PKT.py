...
class PKT(nn.Module):
    ...
    def cosine_similarity_loss(output_net, target_net, eps=0.0000001):
        ...
        output_net_norm = torch.sqrt(
            torch.sum(output_net**2, dim=1, keepdim=True))
        output_net = output_net / (output_net_norm + eps)
        output_net[output_net != output_net] = 0

        target_net_norm = torch.sqrt(
            torch.sum(target_net**2, dim=1, keepdim=True))
        ...
        loss = torch.mean(target_similarity * torch.log(
