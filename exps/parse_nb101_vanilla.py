...
def compute_vanilla_distiller_rank(dataloader,
    ...
    for i in range(len(SAMPLE_HASH)):
        # get random subnet
        snet, acc = get_nb101_model_and_acc(SAMPLE_HASH[i])
...
def compute_diswot_procedure(SAMPLE_HASH,
                             TARGET='cifar10',
                             NUM_CLASSES=10):
    if TARGET == 'cifar100':
    ...
    for i in range(len(SAMPLE_HASH)):
        ...
        # score_sp = -1 * criterion_sp(tfeature[-2],
...
def main_vanilla_zc(TARGET, SAMPLE_HASH):
    if TARGET == 'cifar100':
    ...
    for zc_name in zc_names:
        ...
        for _ in range(times):
            kd, sp, ps = compute_vanilla_zc_rank(
                train_loader, zc_name=zc_name, SAMPLE_HASH=SAMPLE_HASH)
...
def main_kd_zc(TARGET, NUM_CLASSES, SAMPLE_HASH):
    if TARGET == 'cifar100':
        train_loader, val_loader = get_cifar100_dataloaders(batch_size=128,
                                                            num_workers=0)
    elif TARGET == 'ImageNet16-120':
    ...
    for loss_name in loss_name_list:
        ...
        for _ in range(times):
            ...
            kd, sp, ps = compute_vanilla_distiller_rank(
                train_loader, loss_name, SAMPLE_HASH, TARGET, NUM_CLASSES)
            every_time_sp.append(sp)
            every_time_kd.append(kd)
            every_time_ps.append(ps)
...
if __name__ == '__main__':
    ...
    # res = []
    # for i in range(time):
    #     kd, sp, ps = compute_diswot_procedure(SAMPLE_HASH)
    #     res.append(sp)
    # print(res)

    # main_vanilla_zc(TARGET, SAMPLE_HASH)
    main_kd_zc(TARGET, NUM_CLASSES, SAMPLE_HASH)
