...
class ImageFolderSample(datasets.ImageFolder):
    ...
    def __init__(self,
        ...
        if self.is_sample:
            ...
            self.cls_negative = [
                np.asarray(self.cls_negative[i], dtype=np.int32)
                for i in range(num_classes)
            ]
...
def get_dataloader_sample(dataset='imagenet',
                          batch_size=128,
                          num_workers=8,
                          is_sample=False,
                          k=4096):
    ...
    train_set = ImageFolderSample(train_folder,
...
def get_imagenet_dataloader(dataset='imagenet',
                            batch_size=128,
                            num_workers=16,
                            is_instance=False):
    """
    ...
    """
    if dataset == 'imagenet':
        data_folder = '/home/stack/data_sdc/datasets/subImageNet'
        # '/home/inspur/data/imagenet'
        # get_data_folder()
    else:
        raise NotImplementedError('dataset not supported: {}'.format(dataset))
    ...
    if is_instance:
        train_set = ImageFolderInstance(train_folder,
                                        transform=train_transform)
        n_data = len(train_set)
    else:
        train_set = datasets.ImageFolder(train_folder,
                                         transform=train_transform)
