...
from .basic_blocks import BN, Linear, ResBlock
from .super_blocks import PlainNetSuperBlockClass
from .utils import PlainNet
...
class MasterNet(PlainNet):
    def __init__(self,
        ...
        self.fc_linear = Linear(in_channels=self.last_channels,
    ...
    def extract_stage_features_and_logit(self,
                                         x,
                                         target_downsample_ratio=None):
        stage_features_list = []
        ...
        for block_id, the_block in enumerate(self.block_list):
            ...
            if self.dropout is not None:
                dropout_p = float(block_id) / \
                    len(self.block_list) * self.dropout
