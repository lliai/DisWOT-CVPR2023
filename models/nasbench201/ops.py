...
def InChannelWider(module, new_channels, index=None):
    ...
    module.weight = nn.Parameter(torch.cat(
        [weight, weight[:, index, :, :].clone()], dim=1),
                                 requires_grad=True)
...
def OutChannelWider(module, new_channels, index=None):
    ...
    module.weight = nn.Parameter(torch.cat(
        [weight, weight[index, :, :, :].clone()], dim=0),
                                 requires_grad=True)
...
def BNWider(module, new_features, index=None):
    ...
    if module.affine:
        module.weight = nn.Parameter(torch.cat([weight, weight[index].clone()],
                                               dim=0),
                                     requires_grad=True)
        module.bias = nn.Parameter(torch.cat([bias, bias[index].clone()],
                                             dim=0),
                                   requires_grad=True)
        ...
        module.weight.t = 'bn'
        module.bias.t = 'bn'
        module.weight.raw_id = (weight.raw_id
                                if hasattr(weight, 'raw_id') else id(weight))
